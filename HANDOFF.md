...
**💡 今天發什麼：話題框有內容時改走「純主題深化」模式（v5.29.0）**
- 症狀：使用者打了「我想發 AI coding 設計煎熬終於成功」，今天發什麼出來變成「週一工作日的 AI 救命稻草翻身記」，AI 硬塞「週一」「連假」這類今日情境
- 根因：原本不論有沒有 topic，prompt 都是「今天是 X 月 X 日，台灣社群可以發什麼文？」+ 加 _topicNote 補貼，AI 還是會被前綴帶風向
- 修法：拆兩種模式
  - 話題框空白 → 原本的「找今日台灣熱門話題」prompt（保留）
  - 話題框有內容 → 改走「主題深化」prompt：「主題：『X』請幫忙把這個主題深化…⚠️ 不要硬塞今天/週一/連假這類時間情境」
...
- 加新 `<option value="自由發想">📝 自由發想（直接拿話題產）</option>`
- 行為跟 職場梗 / 日常類 一樣（直接 genScript，無 Stage 1，⚡1 點），但 prompt 不帶類別偏向
- typeLabel 映射補一筆「自由發想 → 自由主題」（topic 空白時 AI 自己想話題用）

2. 🎲 重新構思按鈕
- 位置：腳本結果上方、改寫列前面
- 行為：呼叫 `window.genScript()`（不帶 directionHint），自動沿用當前 type+topic
  - 若話題框是 Stage 2 格式 → v5.27.1 修復會偵測並跳過 Stage 1 → 直接重產
  - 若話題框是普通文字 + 自由發想/職場梗 → 直接 Haiku 重產
  - 若 type=今天發什麼 → 會重新跑 Stage 1（合理，因為使用者可能想換 3 方向）
- 點數 badge 跟著 scriptType + topic 動態切（⚡1 或 ⚡2），用 updateRegenBadge() 同步
- updateCostBadges() 末尾呼叫 updateRegenBadge() 確保兩個 badge 同步
